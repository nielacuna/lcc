...
/* strings are created for identifiers, constants, registers and so on.
 * strings are compared often; for example, when a symbol table is searched
 * for an identifier.
 */
...
/* string() makes a copy of the null terminated string 'str'.
 */
char *string(char *str)
{
        char *s;
...
/*stringd() converts 'n' into a string in a private buffer, and 
 * calls stringn, to return the appropriate distinct string.
 */
char *stringd(int n)
{
        char str[25];
        ...
        if (n == INT_MIN)
                ...
                m = -n; /* absolute value if negative */
        else 
                m = n;
...
/* each bucket heads a list of string that shares the same hash value
 */
...
/* we are indexing a character value inside this scatter table. 
 * 1 = 8 bits and 2**8 = 256, so the table size is only the maximum
 * value allowed by a 1 byte character value. */
...
/* stringn maintains the set of distinct strings by saving them in a 
 * 'string table'. it saves exactly one copy of each distinct string and it
 * never removes any string from the table.
 *
 * stringn() adds 'str' to the hash table unless it's already there and
...
char *stringn(char *str, int len)
{
        ...
        h &= NELEMS(buckets)-1; /* h might overshoot the hash table size. so
                                   we make sure it truncats to '1024' if ever
                                   its bigger than 1024. */

        for (p = buckets[h]; p; p = p->link) {
                /* we check to see if a duplicate string is already in the 
                 * hash table. if it exists, no need to install it anymore and 
                 * just return it.*/
                if (len == p->len) {
                        char *s1 = str;
                        char *s2 = p->str;
        ...
        /* string allocates the strings themselves in permanently allocated
         * memory of at least 4KBytes.
         * next points to the next free space where a string can be copied.
         * strlimit is the boundary of the 4kb buffer. 
         *
         * c99 (6.7.9/10) states that if an object has static of thread storage
         * duration and it not initialized explicitly, then:
         *
         * if it has a pointer type, it is initialized to a null pointer.
         * ...
         * */
        static char *next, *strlimit; // initialized to NULL per the C standard.
        ...
        /* now, allocate the actual string object that we will chain on the hash table */
        NEW(p, PERM);
        p->len = len;
        for (p->str = next; str < end; ) { /* strcpy() */
                *next++ = *str++;
        }
        *next++ = 0; /* NULL terminate the string */

        /* insert at head of the bucket list. this is not a circular singly list.
